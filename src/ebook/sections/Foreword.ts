...
import { Heading1Adapter, ParagraphAdapter, Heading3Adapter, Heading4Adapter } from '../adapters/Text'
...
export class ForewordSection implements Section {
    ...
    public async build() {
        ...
        new Heading3Adapter(Yaml.getString('@i18n/WhyShouldRead.yaml')).apply()
        new ParagraphAdapter(Context.getBook().goal)
        new Heading3Adapter(Yaml.getString('@i18n/BookWhoFor.yaml')).apply()
        new ParagraphAdapter(Context.getBook().audience)
        new Heading3Adapter(Yaml.getString('@i18n/Prerequisites.yaml')).apply()
        new ParagraphAdapter(Context.getBook().prerequisites)
        new Heading3Adapter(Yaml.getString('@i18n/BookStructure.yaml')).apply()
        Context.getBook().chapters.forEach(chapter => {
            ...
            title.apply()
            new ParagraphAdapter(chapter.summary)
        })
